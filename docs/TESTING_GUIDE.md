...
docker exec barber-app php artisan test --compact tests/Feature/Auth/AuthenticationTest.php
docker exec barber-app php artisan test --compact --filter=test_admin_can_create_a_service
```
...
├── Support/RefreshMongoDatabase.php       # trait de truncado de colecciones
├── Unit/                                  # (vacío por ahora, reservado para lógica de dominio pura)
└── Feature/
    ├── Auth/            AuthenticationTest, RegistrationTest
    ├── Profile/         ProfileTest
    ├── Security/        RouteGuardAccessTest, SlugRouteBindingTest
    ├── Appointments/    AppointmentManagementTest (admin/recepción), ClientBookingTest
    ├── Payments/        PaymentManagementTest
    ├── Services/        ServiceManagementTest
    ├── Clients/         ClientManagementTest
    ├── Barbers/         BarberManagementTest
    ├── Social/          SocialFeedTest
    ├── Api/             MobileAuthTest, MobileBookingTest (API v1 con token propio, no Sanctum)
    └── Dashboard/       DashboardAccessTest
```
