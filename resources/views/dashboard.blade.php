    ...
    <section class="grid grid-cols-1 lg:grid-cols-12 gap-5">
        ...
        <div class="lg:col-span-5 space-y-4">
            ...
            <div class="rounded-2xl border border-ink/[0.08] bg-card p-5 space-y-4">
                ...
                {{-- Sorteo ganado --}}
                @if($wonRaffle)
                    @php $raffleExpired = $wonRaffle->isExpired(); @endphp
                    <div class="flex items-center gap-2 p-2.5 rounded-lg" style="background:rgba(232,121,249,0.05);border:1px solid rgba(232,121,249,0.15);">
                        ...
                        <div>
                            ...
                            @if($raffleExpired)
                                <p class="text-[8px] text-red-400/70 mt-0.5">Caducó el {{ $wonRaffle->vence_en->format('d/m/Y') }} sin reclamarse</p>
                            @else
                                <p class="text-[8px] text-emerald-400/70 mt-0.5">Válido hasta {{ $wonRaffle->vence_en->format('d/m/Y') }} — coméntalo en tu próxima cita</p>
                            @endif
                        </div>
                    </div>
                @endif
