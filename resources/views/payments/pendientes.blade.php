<x-app-layout>
    ...
    <div class="space-y-5 py-4">
        ...
        @if($payments->isEmpty())
            <div class="rounded-2xl border border-dashed border-ink/10 p-16 text-center">
                <svg class="h-12 w-12 text-ink/5 mx-auto mb-4" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="1" d="M9 12h6m-6 4h6m2 5H7a2 2 0 01-2-2V5a2 2 0 012-2h5.586a1 1 0 01.707.293l5.414 5.414a1 1 0 01.293.707V19a2 2 0 01-2 2z"/></svg>
            ...
            <div class="grid grid-cols-1 lg:grid-cols-2 gap-5">
                @foreach($payments as $payment)
                    ...
                    <div class="ui-card-premium overflow-hidden">
                        <div class="p-5 border-b border-ink/6 flex items-center justify-between">
                            <div>
                        ...
                        <div class="p-5 space-y-4">
                            @if($payment->comprobante_cliente)
                                <a href="{{ \Illuminate\Support\Facades\Storage::disk('public')->url($payment->comprobante_cliente) }}" target="_blank"
                                   class="block rounded-xl border border-ink/10 overflow-hidden bg-black/40">
                            ...
                            <div class="grid grid-cols-2 gap-3 text-sm">
                                <div class="rounded-xl border border-ink/10 bg-ink/5 p-3">
                                    <p class="text-[9px] font-black uppercase tracking-widest text-muted">Monto esperado</p>
                                    <p class="font-black text-ink">${{ number_format($esperado, 2) }}</p>
                                </div>
                            ...
                            @if($payment->ocr_texto)
                                <div class="rounded-xl border border-blue-500/20 bg-blue-500/5 p-3">
                                    ...
                                    <p class="text-[11px] text-ink/70 whitespace-pre-line">{{ Str::limit($payment->ocr_texto, 300) }}</p>
                            ...
                            <div class="flex items-center gap-3 pt-2">
                                ...
                                <div x-data="{ open: false }" class="flex-1">
                                    ...
                                    <div x-show="open" x-cloak @click.outside="open = false" class="mt-3 rounded-xl border border-ink/10 bg-card p-3 space-y-2">
