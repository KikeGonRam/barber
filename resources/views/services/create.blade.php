<x-app-layout>
    <x-slot name="header">
        <div class="flex items-center justify-between gap-3">
            <h2 class="ui-title">Crear servicio</h2>
                ...
                <form method="POST" action="{{ route('services.store') }}" class="space-y-4">
                    ...
                    <div class="ui-form-grid">
                        <div>
                            ...
                            <input id="nombre" name="nombre" value="{{ old('nombre') }}" class="ui-input" required>
                            @error('nombre') <p class="text-sm text-[#525252]">{{ $message }}</p> @enderror
                        </div>

                        <div>
                            <label class="ui-label" for="categoria">Categoria</label>
                            <input id="categoria" name="categoria" value="{{ old('categoria') }}" class="ui-input" required>
                            @error('categoria') <p class="text-sm text-[#525252]">{{ $message }}</p> @enderror
                        </div>

                        <div>
                            <label class="ui-label" for="precio">Precio</label>
                            <input id="precio" type="number" step="0.01" min="0" name="precio" value="{{ old('precio') }}" class="ui-input" required>
                            @error('precio') <p class="text-sm text-[#525252]">{{ $message }}</p> @enderror
                        </div>

                        <div>
                            <label class="ui-label" for="duracion_min">Duracion (min)</label>
                            <input id="duracion_min" type="number" min="5" name="duracion_min" value="{{ old('duracion_min') }}" class="ui-input" required>
                    ...
                    <div>
                        ...
                        <textarea id="descripcion" name="descripcion" class="ui-input">{{ old('descripcion') }}</textarea>
                    ...
                    <div>
                        <label class="inline-flex items-center gap-2 text-sm text-[#2d2d2d]">
                            <input type="checkbox" name="activo" value="1" class="rounded border-[#bfbfbf] text-[#404040] focus:ring-[#8c8c8c]" @checked(old('activo', true))>
                            Activo
                        </label>
                    </div>

                    <div class="ui-toolbar-group pt-2">
                        <button type="submit" class="ui-btn">Guardar servicio</button>
