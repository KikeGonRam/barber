...
>
    ...
    <div 
        ...
        class="mx-auto max-w-2xl transform divide-y divide-ink/5 overflow-hidden rounded-2xl bg-card shadow-2xl ring-1 ring-ink/10 transition-all border border-ink/5"
    >
        <div class="relative">
            ...
            <input 
                x-model="search"
                type="text" 
                class="h-12 w-full border-0 bg-transparent pl-11 pr-4 text-ink placeholder-muted focus:ring-0 sm:text-sm" 
        ...
        <ul class="max-h-96 scroll-py-3 overflow-y-auto p-3 custom-scrollbar" x-show="filteredCommands.length > 0">
            <template x-for="command in filteredCommands" :key="command.id">
                <li>
                    ...
                    >
                        <div class="flex h-10 w-10 flex-none items-center justify-center rounded-lg bg-card/5 border border-ink/5 text-gold group-hover:bg-gold group-hover:text-black transition-all">
                            <svg class="h-6 w-6" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                                <path stroke-linecap="round" stroke-linejoin="round" stroke-width="1.5" :d="command.icon" />
                            </svg>
                        </div>
                        <div class="ml-4 flex-auto">
                            <p class="text-sm font-black text-ink uppercase tracking-widest" x-text="command.name"></p>
        ...
        <div x-show="filteredCommands.length === 0" class="px-6 py-14 text-center sm:px-14">
            ...
            <p class="mt-4 text-sm text-ink font-bold">No se encontraron resultados para "<span x-text="search"></span>"</p>
