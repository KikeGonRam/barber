<x-app-layout>
    <x-slot name="header">
        <div class="flex items-center justify-between gap-3">
            <h2 class="ui-title">Crear cita</h2>
            <span class="ui-badge">Alta rapida</span>
        </div>
    </x-slot>
                ...
                <form method="POST" action="{{ route('appointments.store') }}" class="space-y-4">
                    ...
                    <div class="ui-form-grid">
                        <div>
                            ...
                            <input id="fecha" type="date" name="fecha" value="{{ old('fecha') }}" class="ui-input" required>
                            @error('fecha') <p class="text-sm text-[#525252]">{{ $message }}</p> @enderror
                        </div>
                        <div>
                            <label class="ui-label" for="hora_inicio">Hora inicio</label>
                            <input id="hora_inicio" type="time" name="hora_inicio" value="{{ old('hora_inicio') }}" class="ui-input" required>
                    ...
                    <div class="md:max-w-[50%]">
                        ...
                        <input id="hora_fin" type="time" name="hora_fin" value="{{ old('hora_fin') }}" class="ui-input" required>
                        <p class="ui-field-help">Define fin real para evitar cruces de agenda.</p>
