...
<nav class="flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between mt-6" role="navigation" aria-label="Paginación">
    ...
    <p class="text-[10px] font-black uppercase tracking-[0.2em] text-muted">
        @if ($paginator->firstItem())
            ...
            resultados
        @else
            <span class="text-white">{{ $paginator->count() }}</span> resultados
        @endif
    </p>
    ...
    <div class="flex items-center gap-1" role="group">
        ...
        @if ($paginator->onFirstPage())
            <span aria-disabled="true"
                  class="inline-flex h-8 w-8 items-center justify-center rounded-xl border border-white/5 text-white/15 cursor-not-allowed select-none">
        ...
        @else
            <a href="{{ $paginator->previousPageUrl() }}" rel="prev"
               ...
               class="inline-flex h-8 w-8 items-center justify-center rounded-xl border border-white/10 text-muted hover:text-white hover:border-gold/40 hover:bg-gold/8 active:scale-95 transition-all duration-150">
                <svg class="h-3.5 w-3.5" fill="currentColor" viewBox="0 0 20 20" aria-hidden="true">
                    <path fill-rule="evenodd" d="M12.707 5.293a1 1 0 010 1.414L9.414 10l3.293 3.293a1 1 0 01-1.414 1.414l-4-4a1 1 0 010-1.414l4-4a1 1 0 011.414 0z" clip-rule="evenodd"/>
                </svg>
            </a>
                ...
                @foreach ($element as $page => $url)
                    ...
                    @else
                        <a href="{{ $url }}"
                           ...
                           class="inline-flex h-8 w-8 items-center justify-center rounded-xl border border-white/10 text-muted hover:text-white hover:border-gold/40 hover:bg-gold/8 active:scale-95 text-[10px] font-black transition-all duration-150">
                            {{ $page }}
                        </a>
                    @endif
                @endforeach
