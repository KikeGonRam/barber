...
<nav class="flex items-center justify-between gap-4 mt-6" role="navigation" aria-label="Paginación">
    ...
    @if ($paginator->onFirstPage())
        <span aria-disabled="true"
              class="inline-flex items-center gap-1.5 rounded-xl border border-ink/5 px-3 py-1.5 text-[10px] font-black uppercase tracking-widest text-ink/15 cursor-not-allowed select-none">
    ...
    @else
        <a href="{{ $paginator->previousPageUrl() }}" rel="prev"
           class="inline-flex items-center gap-1.5 rounded-xl border border-ink/10 px-3 py-1.5 text-[10px] font-black uppercase tracking-widest text-muted hover:text-ink hover:border-gold/40 hover:bg-gold/5 active:scale-95 transition-all duration-150">
            <svg class="h-3 w-3" fill="currentColor" viewBox="0 0 20 20"><path fill-rule="evenodd" d="M12.707 5.293a1 1 0 010 1.414L9.414 10l3.293 3.293a1 1 0 01-1.414 1.414l-4-4a1 1 0 010-1.414l4-4a1 1 0 011.414 0z" clip-rule="evenodd"/></svg>
            Anterior
        </a>
    @endif
