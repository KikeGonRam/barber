    ...
    <div class="space-y-5 py-4">
            ...
            <div x-show="open" x-transition:enter="transition ease-out duration-200" x-transition:enter-start="opacity-0 -translate-y-2" x-transition:enter-end="opacity-100 translate-y-0" class="px-6 py-5">
                <form method="GET" action="{{ route('inventory.movements.index') }}">
                    ...
                    <div class="mt-5 flex items-center gap-3">
                        ...
                        @if(count($activeFilters) > 0)
                            <a href="{{ route('inventory.movements.index') }}" class="flex items-center gap-1.5 rounded-xl border border-ink/10 px-4 py-2.5 text-[11px] font-black uppercase tracking-widest text-muted hover:text-ink transition-all">
        ...
        <section>
                            ...
                            <tr class="group">
                                <td>
                                    <div class="flex flex-col">
                                        <span class="font-bold text-ink text-sm">{{ \Carbon\Carbon::parse($mov->fecha)->translatedFormat('d M, Y') }}</span>
                                        <span class="text-[10px] text-muted">{{ \Carbon\Carbon::parse($mov->fecha)->format('H:i') }}</span>
                                    </div>
                                </td>
                                <td class="font-bold text-ink text-sm">{{ $mov->product?->nombre ?? '—' }}</td>
                                <td>
                                ...
                                <td>
                                    <div class="flex items-center gap-2">
                                        @if($mov->user)
                                            <div class="h-6 w-6 rounded-lg bg-ink/5 border border-ink/8 flex items-center justify-center text-[9px] font-black text-gold">
                                                {{ strtoupper(mb_substr($mov->user->name, 0, 2)) }}
                                            </div>
                                            <span class="text-sm text-ink/80">{{ $mov->user->name }}</span>
                                        @else
            ...
            <div class="space-y-3 md:hidden">
                @forelse($movements as $mov)
                    ...
                    <div class="rounded-2xl border border-ink/8 bg-card p-4">
                        <div class="flex items-start justify-between mb-3">
                            <div>
                                <p class="font-bold text-ink text-sm">{{ $mov->product?->nombre ?? '—' }}</p>
                        ...
                        <div class="border-t border-ink/5 pt-3 flex items-center justify-between">
                ...
                @empty
                    <div class="rounded-2xl border border-dashed border-ink/10 p-10 text-center"><p class="text-sm text-muted">Sin movimientos.</p></div>
                @endforelse
            </div>
