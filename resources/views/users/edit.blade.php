<x-app-layout>
    <x-slot name="header">
        <div class="flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between">
            <div>
                <h2 class="ui-title">Ajustar <span class="text-gold">Acceso de Miembro</span></h2>
                <p class="ui-subtitle">Modifica los permisos y credenciales del personal o cliente.</p>
            </div>
            <a href="{{ route('users.index') }}" class="text-[10px] font-black uppercase tracking-widest text-muted hover:text-ink transition">
                ...
                <form method="POST" action="{{ route('users.update', $user) }}" class="space-y-10" x-data="{ role: '{{ old('role', $user->roleNames()->first()) }}' }">
                    ...
                    <div>
                        <div class="mb-6 flex items-center gap-3">
                            ...
                            <h3 class="text-sm font-black text-ink uppercase tracking-widest">Identidad del Usuario</h3>
                        </div>
                    ...
                    <div class="pt-8 border-t border-ink/5">
                        <div class="mb-6 flex items-center gap-3">
                            ...
                            <h3 class="text-sm font-black text-ink uppercase tracking-widest">Asignación de Rol & Permisos</h3>
                        </div>
                        ...
                        <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
                            ...
                            @foreach($roles as $r)
                                <button type="button" 
                                        @click="role = '{{ $r }}'"
                                        :class="role === '{{ $r }}' ? 'border-gold bg-gold/10 text-gold shadow-[0_0_20px_rgba(212,175,55,0.1)]' : 'border-ink/5 bg-ink/5 text-muted'"
                                        class="flex flex-col text-left p-5 rounded-2xl border transition-all hover:border-gold/30 group">
                                    <div class="h-10 w-10 rounded-xl bg-ink/5 border border-ink/10 flex items-center justify-center mb-4 group-hover:text-gold transition-colors">
                    ...
                    <div class="pt-8 border-t border-ink/5">
                        <div class="mb-6 flex items-center gap-3">
                            ...
                            <h3 class="text-sm font-black text-ink uppercase tracking-widest">Seguridad & Contraseña</h3>
                        </div>

                        <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                            <div class="group">
                                <label class="ui-label" for="password">Nueva Contraseña (Opcional)</label>
                            ...
                            <div class="group">
                                <label class="ui-label" for="password_confirmation">Repetir Nueva Contraseña</label>
                                <input id="password_confirmation" type="password" name="password_confirmation" 
                                       class="ui-input !bg-panel border-ink/10 text-ink" 
                                       placeholder="••••••••">
                            </div>
                        </div>
                        <p class="mt-4 text-[9px] text-muted uppercase font-bold italic">Deja los campos de contraseña vacíos si no deseas cambiarla.</p>
                    </div>
                    ...
                    <div class="pt-10 border-t border-ink/5 flex justify-end">
                        <button type="submit" class="ui-btn px-16 py-4 text-[11px] uppercase tracking-[0.2em] shadow-lg shadow-gold/20">
                            Guardar Cambios de Usuario <span class="ml-2 opacity-50">&rarr;</span>
                        </button>
                    </div>
                </form>
            </section>
