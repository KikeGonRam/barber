    ...
    <x-slot name="header">
        <div class="flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between">
            ...
            <a href="{{ route('barber.agenda') }}"
               class="flex items-center gap-1.5 text-[10px] font-black uppercase tracking-widest text-muted hover:text-ink transition">
    ...
    <div class="space-y-5 py-4" x-data="scheduleManager()">
        ...
        <section class="flex flex-wrap items-center gap-3">
            ...
            <button type="button" @click="copyFirstToAll()"
                    class="inline-flex items-center gap-1.5 rounded-xl border border-ink/10 px-3.5 py-2 text-[10px] font-black uppercase tracking-widest text-muted hover:text-gold hover:border-gold/30 transition-all">
            ...
            <form method="POST" action="{{ route('barber.schedule.update') }}" id="scheduleForm">
                ...
                <div class="divide-y divide-ink/5">
                    @foreach($schedules as $schedule)
                        ...
                        <div class="group px-6 py-5 hover:bg-ink/[0.02] transition-colors"
                            ...
                            <div class="flex flex-col sm:flex-row sm:items-center gap-5">
                                ...
                                <div class="flex items-center gap-3 sm:w-40 shrink-0">
                                    <div class="h-10 w-10 rounded-xl flex items-center justify-center text-xs font-black border transition-all"
                                         :class="days[{{ $schedule->day_of_week }}].working
                                             ...
                                             : 'bg-ink/5 border-ink/8 text-muted'">
                                        {{ strtoupper(mb_substr($days[$schedule->day_of_week], 0, 2)) }}
                                    </div>
                                    <div>
                                        <p class="text-sm font-black text-ink uppercase tracking-wide">{{ $days[$schedule->day_of_week] }}</p>
                                    ...
                                    <label class="relative inline-flex cursor-pointer items-center gap-3 select-none">
                                        ...
                                        <div class="relative h-6 w-11 rounded-full border transition-all duration-300
                                            ...
                                            <div class="absolute left-1 top-1 h-4 w-4 rounded-full transition-all duration-300
                                                 ...
                                                 :class="days[{{ $schedule->day_of_week }}].working ? 'translate-x-5 bg-gold' : 'translate-x-0 bg-ink/30'">
                                            </div>
                                        </div>
                                ...
                                <div class="flex-1 grid grid-cols-2 gap-4"
                                    ...
                                    <div>
                                        ...
                                        <input type="time"
                                               ...
                                               class="h-10 w-full rounded-xl border border-ink/10 bg-black/30 px-3 text-sm text-ink focus:border-gold/50 focus:outline-none transition-all">
                                    </div>
                                    <div>
                                ...
                                <div class="shrink-0">
                                    <button type="button"
                                            ...
                                            class="h-9 w-9 rounded-xl border border-ink/8 bg-ink/3 flex items-center justify-center text-muted hover:text-gold hover:border-gold/30 transition-all"
                ...
                {{-- ── RESUMEN SEMANAL ─── --}}
                <div class="px-6 py-4 border-t border-ink/5 bg-black/20 flex flex-wrap gap-6 items-center justify-between">
                    <div class="flex flex-wrap gap-4">
                        <div>
                            <p class="text-[9px] text-muted uppercase tracking-wider">Días laborales</p>
                            <p class="text-xl font-black text-ink" x-text="workingDaysCount()"></p>
                        </div>
                        <div>
