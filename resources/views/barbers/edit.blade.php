        ...
        <div class="flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between">
            ...
            <div class="flex items-center gap-3">
                <a href="{{ route('barbers.performance', $barber) }}"
                   class="flex items-center gap-1.5 rounded-xl border border-white/10 px-4 py-2.5 text-[10px] font-black uppercase tracking-widest text-muted hover:text-gold hover:border-gold/30 transition-all">
                    <svg class="h-3.5 w-3.5" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M9 19v-6a2 2 0 00-2-2H5a2 2 0 00-2 2v6a2 2 0 002 2h2a2 2 0 002-2zm0 0V9a2 2 0 012-2h2a2 2 0 012 2v10m-6 0a2 2 0 002 2h2a2 2 0 002-2m0 0V5a2 2 0 012-2h2a2 2 0 012 2v14a2 2 0 01-2 2h-2a2 2 0 01-2-2z"/></svg>
                    Ver Stats
                </a>
                <a href="{{ route('barbers.index') }}"
                   class="flex items-center gap-1.5 text-[10px] font-black uppercase tracking-widest text-muted hover:text-white transition-all">
            ...
            <form method="POST"
                ...
                <aside class="space-y-5">
                    ...
                    <div class="ui-card-premium p-6 flex flex-col items-center gap-5">
                        <div class="relative">
                            ...
                            {{-- Estado dot --}}
                            <div class="absolute -bottom-1 -right-1 h-6 w-6 rounded-full border-[3px] border-[#0f0f0f] {{ $barber->activo ? 'bg-emerald-500' : 'bg-white/20' }}"></div>
                        </div>

                        <div class="text-center">
                            <p class="font-black text-white text-base">{{ $barber->user?->name }}</p>
                        ...
                        <div class="w-full">
                            ...
                            <label class="flex flex-col items-center gap-2 w-full rounded-xl border border-dashed border-white/10 bg-black/20 px-4 py-4 cursor-pointer hover:border-gold/30 hover:bg-gold/5 transition-all group">
                    ...
                    {{-- Toggle activo --}}
                    <div class="rounded-2xl border border-white/8 bg-[#111] p-5" x-data="{ active: {{ $barber->activo ? 'true' : 'false' }} }">
                        ...
                        <label class="flex items-center justify-between cursor-pointer group">
                            ...
                            <div class="relative">
                                ...
                                <div class="h-7 w-12 rounded-full border transition-all duration-300"
                                     :class="active ? 'bg-emerald-500/20 border-emerald-500/40' : 'bg-white/5 border-white/10'">
                                    <div class="absolute top-1.5 h-4 w-4 rounded-full transition-all duration-300"
                                         :class="active ? 'left-7 bg-emerald-400' : 'left-1.5 bg-white/30'">
                                    </div>
                                </div>
                            </div>
                        </label>
                ...
                <main class="lg:col-span-2 space-y-5">
                        ...
                        <div class="grid grid-cols-1 sm:grid-cols-2 gap-5">
                            <div>
                                ...
                                <input name="name" value="{{ old('name', $barber->user?->name) }}"
                                       class="h-11 w-full rounded-xl border border-white/10 bg-black/30 px-4 text-sm text-white focus:border-gold/50 focus:outline-none transition-all"
                    ...
                    <div class="flex items-center justify-between gap-4 pt-1">
                        <a href="{{ route('barbers.index') }}"
                           class="text-[10px] font-black uppercase tracking-widest text-muted hover:text-white transition">
                            Cancelar
                        </a>
