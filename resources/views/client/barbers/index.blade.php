<x-app-layout>
    ...
    <div class="py-8">
        <div class="mx-auto max-w-5xl px-4 sm:px-6 lg:px-8">
            ...
            @if($barbers->isEmpty())
                <div class="py-20 text-center rounded-3xl border border-dashed border-ink/5">
                ...
                <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-6">
                    ...
                    @foreach($barbers as $barber)
                        ...
                        <a href="{{ route('client.barberos.show', $barber) }}"
                           class="group block rounded-3xl border border-ink/5 bg-ink/[0.02] overflow-hidden transition-all hover:border-gold/30 hover:shadow-xl hover:shadow-gold/5 hover:-translate-y-1">
                            ...
                            <div class="relative h-52 sm:h-56 overflow-hidden bg-black">
                                ...
                                <div class="absolute top-3 right-3 flex items-center gap-1 rounded-full bg-black/50 border border-ink/10 backdrop-blur-sm px-2.5 py-1">
                                    <svg class="h-3 w-3 text-gold/70" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                                        <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M9 12l2 2 4-4m6 2a9 9 0 11-18 0 9 9 0 0118 0z"/>
                                    </svg>
                                    <span class="text-[9px] font-black text-ink/70">{{ $barber->citas_completadas }} citas</span>
                                </div>
                                @endif
                            </div>
                            ...
                            <div class="p-5">
                                <div class="flex items-start justify-between gap-3">
                                    <div>
                                        <h3 class="text-base font-black text-ink uppercase tracking-tight group-hover:text-gold transition-colors">
                                            {{ $barber->user?->name }}
                                        </h3>
                                        <p class="text-[9px] font-bold text-muted uppercase tracking-[0.2em] mt-0.5">Master Groomer</p>
                                    </div>
                                    <div class="h-7 w-7 rounded-xl bg-ink/5 border border-ink/10 flex items-center justify-center group-hover:bg-gold group-hover:border-gold transition-all flex-shrink-0">
