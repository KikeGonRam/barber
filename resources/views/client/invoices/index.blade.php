        ...
        <div class="mx-auto max-w-5xl px-4 sm:px-6 lg:px-8 space-y-8">
            ...
            <div class="grid grid-cols-2 sm:grid-cols-3 gap-4">
                <div class="rounded-2xl border border-ink/5 bg-ink/[0.02] p-5">
                    ...
                    <p class="text-3xl font-black text-ink mt-2">{{ $payments->total() }}</p>
                </div>
                ...
                <div class="rounded-2xl border border-ink/5 bg-ink/[0.02] p-5 col-span-2 sm:col-span-1">
                    <p class="text-[9px] font-black uppercase tracking-[0.25em] text-muted">Total de citas</p>
                    <p class="text-3xl font-black text-ink mt-2">{{ $totalCitas }}</p>
                </div>
            </div>
                    ...
                    @foreach($payments as $payment)
                        @php
                            ...
                            $metodoBadge = match($payment->metodo_pago) {
                                ...
                                default         => ['label' => ucfirst($payment->metodo_pago ?? '—'), 'cls' => 'border-ink/10 bg-ink/5 text-muted'],
                            };
                        @endphp

                        <div class="group rounded-2xl border border-ink/5 bg-ink/[0.02] overflow-hidden hover:border-gold/15 transition-all">
                            <div class="flex flex-col sm:flex-row sm:items-center gap-5 p-5">
                                ...
                                <div class="flex-shrink-0 flex items-center gap-4">
                                    ...
                                    <div>
                                        ...
                                        <p class="text-xs font-black text-ink font-mono mt-0.5">#{{ strtoupper(mb_substr((string)$payment->id, -8)) }}</p>
                                    </div>
                                </div>

                                <!-- Center: service + barber + date -->
                                <div class="flex-1 min-w-0 space-y-1">
                                    <p class="text-sm font-black text-ink uppercase leading-tight truncate">
                                ...
                                <div class="flex sm:flex-col items-center sm:items-end justify-between sm:justify-center gap-3 sm:gap-2 flex-shrink-0">
                                    ...
                                    <a href="{{ route('client.facturas.download', $payment) }}"
                                       class="flex items-center gap-1.5 px-4 py-3 rounded-xl border border-ink/10 bg-ink/5 text-[9px] font-black uppercase text-muted hover:border-gold/30 hover:text-gold hover:bg-gold/[0.04] transition-all group/dl min-h-[44px]">
                            ...
                            @if($appt && $appt->productos && count($appt->productos) > 0)
                                <div class="px-5 pb-4 flex flex-wrap gap-2">
                                    @foreach($appt->productos as $prod)
                                        <span class="px-2 py-0.5 rounded-full bg-ink/[0.04] border border-ink/5 text-[8px] font-black text-muted uppercase">
